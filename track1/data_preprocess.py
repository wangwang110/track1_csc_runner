...
def encode_parallel_data(config, parallel_data, normalizer, tokenizer, max_len):
    ...
    for item in tqdm(parallel_data):
        ...
        token_labels = [0 if data_sample['input_ids'][i] == data_sample['trg_ids'][i] else 1
                        for i in range(len(data_sample['input_ids'] ))]
        data_sample['token_labels'] = token_labels
